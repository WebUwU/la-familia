import { db, ref, set, get, push, onValue, remove, update, RANKS, SUBROLES, getSession, setSession, clearSession, esc, parseDMTime, formatDT, getSubroleLabels, initDB, generatePin } from './firebase-config.js';
...
window.addMember = async function() {
  ...
  var pin = generatePin();
  await push(ref(db, 'members'), { rank: rank, icName: icName, discord: discord, discordId: discordId, absence: null, sanctions: null, notes: '', subroles: null, warns: 0, pin: pin });
  alert('Mitglied erstellt! PIN für Abmeldung: ' + pin);
  closeModal('addModal');
};
...
window.openDetail = function(key) {
  ...
  // PIN display (only for admins)
  if (isLoggedIn && m.pin) {
    html += '<div style="margin-bottom:16px;display:flex;align-items:center;gap:10px"><div class="detail-field" style="display:inline-flex;align-items:center;gap:10px;padding:8px 14px"><div class="detail-field-label" style="margin:0">Abmelde-PIN</div><div class="detail-field-value" style="font-family:\'JetBrains Mono\',monospace;font-size:1rem;color:var(--accent-gold);letter-spacing:3px">' + esc(m.pin) + '</div></div><button class="btn btn-sm" onclick="copyPin(\'' + esc(m.pin) + '\',this)" style="font-size:.72rem">📋 Kopieren</button></div>';
  }
...
// ==================== COPY PIN ====================
window.copyPin = function(pin, btn) {
